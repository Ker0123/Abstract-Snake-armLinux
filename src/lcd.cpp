...
bool LCD_Manager::is_in_range(Vector2<int> pos) const
{
...
void LCD_Manager::Load_FontImage(const char *font_image, const char *font_index_text, Vector2<int> cut)
{
...
void LCD_Manager::clear_layer(int index)
{
    ...
    memset(layer.pixels, 0x0000, 800 * 480 * sizeof(pixel));
}
...
void LCD_Manager::erase_layer(int index, Vector2<int> start_pos, Vector2<int> size)
{
    Layer layer = layers[index];
    for (int x = start_pos.x; x < start_pos.x + size.x; x++)
        for (int y = start_pos.y; y < start_pos.y + size.y; y++)
        {
            layer.pixels[y * 800 + x] = 0x0000;
        }
}

void LCD_Manager::erase_layer(const string &name, Vector2<int> start_pos, Vector2<int> size)
{
    auto it = layer_map.find(name);
    if (it == layer_map.end())
    {
...
void LCD_Manager::Print_Char(int layer_index, Vector2<int> scr_pos, wchar_t c, float scale, pixel color)
{
    ...
    Vector2<int> img_pos = (it == font_map.end()) ? font_map['~' + 1] : it->second;
    ...
    if (abs(scale - 1.0) < 0.001) // 不缩放
    {
        for (int l = 0; l < font_height; l++)
        {
            ...
            {
                if (!is_in_range(scr_pos + Vector2<int>{c, l}))
                {
                    continue;
                }
    ...
    Vector2<int> scr_size_new = {int(font_width * scale), int(font_height * scale)};
    Vector2<int> fit_to_img;
...
void LCD_Manager::Print_Text(int layer_index, Vector2<int> start_pos, string text, float scale, pixel color, Vector2<int> LC_adjust)
{
    if (layer_index < 0 || layer_index >= layers.size())
    {
        throw out_of_range("Layer index out of range");
    ...
    Vector2<int> word_size = {int(font.word_size.x * scale + LC_adjust.x), int(font.word_size.y * scale + LC_adjust.y)}; // 计算缩放后的字体大小，用以移动打印位置
    Vector2<int> write_pos = start_pos;
...
void LCD_Manager::Print_Text(int layer_index, Vector2<int> start_pos, Vector2<int> end_pos, string text, float scale, pixel color, Vector2<int> LC_adjust)
{
    if (layer_index < 0 || layer_index >= layers.size())
    {
        throw out_of_range("Layer index out of range");
...
void LCD_Manager::Print_Text(const string &layer_name, Vector2<int> start_pos, const string &text, float scale, pixel color, Vector2<int> LC_adjust)
{
...
void LCD_Manager::Print_Text(const string &layer_name, Vector2<int> start_pos, Vector2<int> end_pos, const string &text, float scale, pixel color, Vector2<int> LC_adjust)
{
    auto it = layer_map.find(layer_name);
    if (it == layer_map.end())
    {
...
void LCD_Manager::draw_image(int layer_index, Vector2<int> loc, int image_index, Vector2<float> pivot)
{
    // 获取图层和图像
    ...
    Vector2<int> beg(loc.x, loc.y);
    Vector2<int> end(beg.x + image.width, beg.y + image.height);
    ...
    // 世界坐标系->图片本地坐标系
    Vector2<int> local_beg(beg.x - loc.x, beg.y - loc.y);
    Vector2<int> local_end(end.x - loc.x, end.y - loc.y);

    // 以图层为主体，将图片的像素点一一对应，在图层上绘制。
    Vector2<int> layer_pos(beg.x, beg.y);
    Vector2<int> image_pos(local_beg.x, local_beg.y);
...
void LCD_Manager::draw_image(int layer_index, Vector2<int> loc, int image_index, Vector2<float> pivot, char modi_code)
{
    // 获取图层和图像
    if (layer_index < 0 || layer_index >= layers.size() || image_index < 0 || image_index >= images.size())
    {
    ...
    switch (modi_code)
    {
    case 0x00: // 不做操作, 遍历顺序: 第1行左到右, 第2行左到右, ..., 第n行左到右
        for (layer_pos.y = beg.y; layer_pos.y < end.y; layer_pos.y++)
            for (layer_pos.x = beg.x; layer_pos.x < end.x; layer_pos.x++)
                layer.pixels[layer_pos.y * 800 + layer_pos.x] = image.data[n++];
        break;
    case 0x01: // 旋转90°, 遍历顺序: 第n列上到下, 第n-1列上到下, ..., 第1列上到下
        for (layer_pos.x = end.x - 1; layer_pos.x >= beg.x; layer_pos.x--)
            for (layer_pos.y = beg.y; layer_pos.y < end.y; layer_pos.y++)
                layer.pixels[layer_pos.y * 800 + layer_pos.x] = image.data[n++];
        break;
    case 0x02: // 旋转180°, 遍历顺序: 第n行右到左, 第n-1行右到左, ..., 第1行右到左
        for (layer_pos.y = end.y - 1; layer_pos.y >= beg.y; layer_pos.y--)
            for (layer_pos.x = end.x - 1; layer_pos.x >= beg.x; layer_pos.x--)
                layer.pixels[layer_pos.y * 800 + layer_pos.x] = image.data[n++];
        break;
    case 0x03: // 旋转270°(-90), 遍历顺序: 第1列下到上, 第2列下到上, ..., 第n列下到上
        for (layer_pos.x = beg.x; layer_pos.x < end.x; layer_pos.x++)
            for (layer_pos.y = end.y - 1; layer_pos.y >= beg.y; layer_pos.y--)
                layer.pixels[layer_pos.y * 800 + layer_pos.x] = image.data[n++];
        break;
    case 0x11: // 左右翻转, 遍历顺序: 第1行右到左, 第2行右到左, ..., 第n行左到右
        for (layer_pos.y = beg.y; layer_pos.y < end.y; layer_pos.y++)
            for (layer_pos.x = end.x - 1; layer_pos.x >= beg.x; layer_pos.x--)
                layer.pixels[layer_pos.y * 800 + layer_pos.x] = image.data[n++];
        break;
    case 0x12: // 上下翻转, 遍历顺序: 第n行左到右, 第n-1行左到右, ..., 第1行左到右
        for (layer_pos.y = end.y - 1; layer_pos.y >= beg.y; layer_pos.y--)
            for (layer_pos.x = beg.x; layer_pos.x < end.x; layer_pos.x++)
                layer.pixels[layer_pos.y * 800 + layer_pos.x] = image.data[n++];
        break;
...
void LCD_Manager::draw_image(const string &layer_name, Vector2<int> loc, const string &image_name, Vector2<float> pivot)
{
...
void LCD_Manager::draw_image(const string &layer_name, Vector2<int> loc, int image_index, Vector2<float> pivot)
{
...
void LCD_Manager::draw_image(const string &layer_name, Vector2<int> loc, const string &image_name, Vector2<float> pivot, char modi_code)
{
    /**
     * 0x00: 不做操作
     * 旋转
    ...
    int layer_index = layer_map[layer_name];
    int image_index = image_map[image_name];
    draw_image(layer_index, loc, image_index, pivot, modi_code);
}

void LCD_Manager::draw_pixel(int layer_index, Vector2<int> loc, pixel color)
{
...
void LCD_Manager::draw_line(int layer_index, Vector2<int> start, Vector2<int> end, pixel color)
{
...
void LCD_Manager::draw_line(int layer_index, Vector2<int> start, Vector2<int> end, int thickness, pixel color)
{
    int dx = abs(end.x - start.x);
    int dy = abs(end.y - start.y);
    int sx = start.x < end.x ? 1 : -1;
    ...
    while (true)
    {
        for (int i = -half_thickness; i <= half_thickness; i++)
        {
            ...
            {
                draw_pixel(layer_index, Vector2<int>{start.x + i, start.y + j}, color);
            }
        }
        if (start.x == end.x && start.y == end.y)
        {
...
void LCD_Manager::draw_rect(int layer_index, Vector2<int> top_left, Vector2<int> bottom_right, pixel color)
{
    for (int x = top_left.x; x < bottom_right.x; x++)
    {
        for (int y = top_left.y; y < bottom_right.y; y++)
        {
            draw_pixel(layer_index, Vector2<int>{x, y}, color);
        }
    }
}

void LCD_Manager::draw_circle(int layer_index, Vector2<int> center, int radius, pixel color)
{
    int dx, dy;
