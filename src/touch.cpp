    ...
    while (!tm->should_stop)
    {
        ...
        if (!is_touch)
        {
            ...
            if (Ievent.type == EV_KEY && Ievent.code == BTN_TOUCH && Ievent.value == 1)
            {
                ...
                ie3.pop();
                tm->events.push_back({1, {ieX.value * 800 / 1024, ieY.value * 480 / 600}, {ieT.time.tv_sec - base_time_sec, ieT.time.tv_usec}});
                is_touch = true;
            }
        }
        if (is_touch)
        {
            ...
            {
               tm->events.push_back({2, {tm->events.back().pos.x, Ievent.value * 480 / 600}, {Ievent.time.tv_sec - base_time_sec, Ievent.time.tv_usec}});
            }
...
Vector2<int> Touch_Manager::Get_Start_Position()
{
...
Vector2<int> Touch_Manager::Get_Current_Position()
{
...
Vector2<int> Touch_Manager::Get_Direction(int tms, bool consider_last_touch)
{
    // 事件量不够，返回空。
    ...
    // 已经至少有两个操作，计算能否直接返回
    Vector2<int> end_pos = events[size_now - 1].pos;
    int end_time = events[size_now - 1].time.tv_sec * 1000 + events[size_now - 1].time.tv_usec / 1000;
    Vector2<int> start_pos = events[size_now - 2].pos;
    ...
    {
        return Vector2<int>{end_pos.x - start_pos.x, end_pos.y - start_pos.y};
    }
...
float Touch_Manager::Get_Speed(int tms, bool consider_last_touch)
{
    ...
    if (size_now == 2 || end_time - start_time > tms)
    {
        Vector2<int> dxy = end_pos - start_pos;
